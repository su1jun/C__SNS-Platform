...
const Form: React.FC<FormProps> = ({ 
  ...
  const registerModal = useRegisterModal(); // Manage register modal state
  const loginModal = useLoginModal(); // Manage login modal state

  const { data: currentUser } = useCurrentUser(); // Fetch current user data
  const { mutate: mutatePosts } = usePosts(); // Get mutate function for posts
  const { mutate: mutatePost } = usePost(postId as string); // Get mutate function for a specific post

  const [body, setBody] = useState(''); // State for input text
